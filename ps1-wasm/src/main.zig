...
/// TEMPORARY. Master switch for the Crash "Jungle Rollers" scaffolding below
/// (the kernel-integrity scan, the per-instruction PC ring and the fault
/// report). Measuring what it costs the browser needs a build with it off.
...
//      and if so with the PC and the DMA3/CD state at the time, which says
//      whether a CPU store or an in-flight CD transfer landed there
