...
- **`disc.zig` and `cdrom.zig` disagree on the MODE2 sub-header offset.** `disc.zig`'s `readSectorRaw` skips `16` bytes in its non-2048 branch; `cdrom.zig`'s equivalent computation uses `12`/`24` for what should be the same thing. Found during P5 by the implementer, which correctly left it alone under the no-bug-fixes rule. Both paths currently work on the discs in `games/`, so if one is wrong it is wrong in a way no booted title has exercised — worth resolving against PSX-SPX's sector layout before trusting either number in new code.
