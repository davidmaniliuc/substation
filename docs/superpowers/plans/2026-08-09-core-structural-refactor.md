...
- **RESOLVED 2026-08-11 (commit `2f138a9`): the `disc.zig`/`cdrom.zig` sub-header offset disagreement.** Both numbers are correct, for different sector modes: Mode 1 user data starts at 010h (16), Mode 2 Form 1 at 018h (24), and the sector's own mode byte at raw offset 15 picks between them. `cdrom.zig` took 24 unconditionally, which broke every MODE1/2352 disc; `games/` had no MODE1 PlayStation title to catch it.
