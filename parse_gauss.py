...
text = """
  ...
  000Fh,0010h,0011h,0012h,0013h,0015h,0016h,0018h ;
  0019h,001Bh,001Ch,001Eh,0020h,0021h,0023h,0025h ;
  ...
  0520h,053Bh,0556h,0572h,058Eh,05AAh,05C7h,05E4h ;
  0601h,061Fh,063Eh,065Ch,067Ch,069Bh,06BBh,06DCh ;
  ...
  2129h,217Fh,21D5h,222Ch,2282h,22DAh,2331h,2389h ;
  23E1h,2439h,2492h,24EBh,2545h,259Eh,25F8h,2653h ;
  ...
  4DF7h,4E3Eh,4E84h,4EC9h,4F0Eh,4F52h,4F95h,4FD7h ;
  5019h,505Ah,509Ah,50DAh,5118h,5156h,5194h,51D0h ;
...
"""
values = []
for val in re.findall(r'(-?[0-9A-Fa-f]{3,4})h', text):
    values.append(int(val, 16) if not val.startswith('-') else int(val[1:], 16) * -1)

with open('ps1-core/src/spu_gauss.zig', 'w') as f:
    f.write(f"pub const spu_gauss = [{len(values)}]i16{{\n")
    for i in range(0, len(values), 8):
        f.write("    " + ", ".join(str(v) for v in values[i:i+8]) + ",\n")
    f.write("};\n")

print(f"Written {len(values)} values")
