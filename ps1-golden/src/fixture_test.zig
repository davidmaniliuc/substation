...
test "fixture: the committed synthetic fixture still matches its generator" {
    // Without this, the executable bridge gate compares Swift's ShadowVram
    // against a FROZEN snapshot of Zig's behaviour rather than against Zig:
    // synthetic.zig is imported only by main.zig, so a change to vram.zig's
    // movers, to recorder.zig's run-coalescing, to Kind's ordinals or to
    // Command's field order would move the generated bytes, leave the
    // committed file stale, and every suite would stay green. It is also the
    // Zig half of the ordinal pin — a reordered Kind shears the kind byte of
    // every record here.
    //
    // If this fails, the committed fixture is stale or the core moved. DO NOT
    // regenerate it to make it pass; that is the bug this test exists to
    // catch.
    const a = std.testing.allocator;
...
test "fixture: the primitives fixture has nine frames in the documented order" {
    ...
    try std.testing.expectEqual(@as(usize, 9), parsed.frames.len);
